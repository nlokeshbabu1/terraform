...
resource "aws_instance" "web-server" {
  ...
  subnet_id = aws_subnet.dpw-private_subent_01.id
...
resource "aws_subnet" "dpw-private_subent_01" {
    vpc_id = aws_vpc.dpw-vpc.id
    cidr_block = "10.0.2.0/24"
    map_public_ip_on_launch = "true"
    availability_zone = "ap-south-1b"
    tags = {
      Name = "dpw-private_subent_01"
    }
}
...
resource "aws_route_table" "dpw-private-rt" {
    vpc_id = aws_vpc.dpw-vpc.id
    route {
        cidr_block = "0.0.0.0/0"
        gateway_id = aws_internet_gateway.dpw-igw.id
...
resource "aws_route_table_association" "dpw-rta-private-subent-1" {
    subnet_id = aws_subnet.dpw-private_subent_01.id
    route_table_id = aws_route_table.dpw-private-rt.id
}
